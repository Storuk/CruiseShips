...
public class CruiseDao {
    ...
    public static Cruise date_validation(Date start_date_cruise, Date end_date_cruise, int ship_id) throws ClassNotFoundException {
        ...
        try(Connection con = cm.getConnection();
            PreparedStatement pst = con.prepareStatement("select * from cruise where (start_cruise < ? and ? < end_cruise or start_cruise < ? and ? < end_cruise) and ship_id = ?")) {
            pst.setDate(1, start_date_cruise);
            pst.setDate(2, start_date_cruise);
            pst.setDate(3, end_date_cruise);
